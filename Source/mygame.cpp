...
namespace game_framework
{
	...
	void CGameStateRun::OnBeginState()
	{
		...
		for (int i = 0; i < 10; i++)
		{
			items[i].SetTopLeft(itemX[i], itemY[i]);
		}
		/////////////
	}
	...
	void CGameStateRun::OnInit()	// 遊戲的初值及圖形設定
	{
		...
		////////////////
		const string rootItems = "Bitmaps\\items\\item";
		for (int i = 0; i < 10; i++)
		{
			filePath = rootItems + to_string(i) + ".bmp";
			strcpy(test, filePath.c_str());
			items[i].LoadBitmap(test, RGB(255, 255, 255));
		}
		///////////////
	}
	...
	void CGameStateRun::OnLButtonUp(UINT nFlags, CPoint point)	// 處理滑鼠的動作
	{
		...
		if (mapNow == 20)
		{
			for (int i = 0; i < 10; i++)
			{
				if (point.x > growButtonXY[i][0] && point.x < growButtonXY[i][1] && point.y > growButtonXY[i][2] && point.y < growButtonXY[i][3] && itemBlock[i] == false && money.GetInteger() >= 5000 && itemFlag[i] < 4)//
				{
					itemFlag[i]++;
					itemBlock[i] = true;
					money.Add(-5000);
	...
	void CGameStateRun::OnShow()
	{
		...
		if ((mapNow >= 1 && mapNow <= 14) || mapNow == 20)//所有地圖內顯示等級 金錢  經驗值等
		{
			level.ShowBitmapL();
