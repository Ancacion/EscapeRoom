...
namespace game_framework
{
	...
	void CGameStateRun::OnLButtonDown(UINT nFlags, CPoint point)  // 處理滑鼠的動作
	{
		...
		for (int i = 0; i < MAP_SIZE; i++)
		{
			for (int j = 1; j < OBJ_SIZE; j++)
			{
				if (mapNow == i + 1 && j == 65 && point.x < mapArr[i][j]->Left() + mapArr[i][j]->Width() && point.y > mapArr[i][j]->Top() && point.y < mapArr[i][j]->Top() + mapArr[i][j]->Height())
				{//音源鍵 控制音效
					mapArr[i][65]->OnMove();
					if (controlSound == 1)
					{
	...
	void CGameStateRun::OnMouseMove(UINT nFlags, CPoint point)	// 處理滑鼠的動作
	{
		...
		for (int i = 0; i < 8; i++)
		{
			if (point.y > 450 && point.x > (i * 80) && point.x < (i * 80) + 80)
			{
				for (int j = 0; j < 8; j++)
				{
					for (int k = 0; k < 15; k++)
